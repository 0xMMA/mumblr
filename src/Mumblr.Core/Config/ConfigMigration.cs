...
/// <summary>
/// Brings a loaded config up to the current shipped defaults.
///
/// The file is written in full on first run, defaults included, so every install carries a copy
/// of whatever prompt shipped at the time - and keeps it through every update. A stored value
/// that still equals an older shipped default was never edited and follows the current default.
...
/// commit, or no existing install ever sees the new one. <see cref="Fingerprint(string)"/> is how
/// you compute it, and every entry is pinned against its literal text in the tests - a wrong
/// fingerprint is otherwise silent. Layout is ignored, so the indentation of a raw string literal
/// does not matter.
/// </summary>
public static class ConfigMigration
{
    ...
    /// <summary>How a new legacy entry above is computed. Layout-insensitive; see the tests.</summary>
